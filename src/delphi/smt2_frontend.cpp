...
int negation_solver(std::istream &in, decision_proceduret &solver, decision_proceduret &solver2, oracle_solvert::repr_syntht repr_type, message_handlert &message_handler)
{
  messaget message(message_handler);

  negation_oracle_solvert oracle_solver(solver, solver2, repr_type, message_handler);
...
int solver(std::istream &in, decision_proceduret &solver, negation_oracle_solvert::repr_syntht repr_type, message_handlert &message_handler)
{
  messaget message(message_handler);

  oracle_solvert oracle_solver(solver, repr_type, message_handler);
  smt2_solvert smt2_solver{in, oracle_solver};
...
// warning simplify won't work with oracles until freezing is implemented.
  if (cmdline.isset("negation-solver"))
  {
    if (cmdline.isset("bitblast") && cmdline.isset("simplify"))
    {
      ...
      return negation_solver(in, subsolver, subsolver2, oracle_repr_type, message_handler);
    }
    ...
    else
    {
      ...
      smt2_dect negation_subsolver(
          ...
          logic, smt2_dect::solvert::CVC4, message_handler);
      return negation_solver(in, subsolver, negation_subsolver, oracle_repr_type, message_handler);
    }
  }
  else
  {
    if (cmdline.isset("bitblast") && cmdline.isset("simplify"))
    {
      satcheck_minisat_simplifiert satcheck_minisat_no_simplifier(message_handler);
      boolbvt subsolver(ns, satcheck_minisat_no_simplifier, message_handler);
      return solver(in, subsolver, oracle_repr_type, message_handler);
    }
    else if (cmdline.isset("bitblast"))
    {
      satcheck_minisat_no_simplifiert satcheck_minisat_no_simplifier(message_handler);
