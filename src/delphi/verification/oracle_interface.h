...
class oracle_interfacet : public verifyt
{
 public:
  oracle_interfacet(namespacet &_namespace, 
                message_handlert &_ms, oracle_solvert::oracle_repr_optionst _repr_options, bool _bitblast) :
                bitblast(_bitblast),
                repr_options(_repr_options),
                ns(_namespace),
                log(_ms){};
  ...
  void call_second_order_oracles(oracle_solvert &solver, const solutiont &solution, const problemt& problem);
  void get_oracle_constraints(
    const counterexamplet &,
    const oracle_constraint_gent &,
    problemt &problem, const solutiont &solution);
